  ...
  return (
    <div className="space-y-1">
      {items.map((it, i) => {
        ...
        return (
          <div key={i} className={isOpen ? "rounded-lg" : "border-b border-slate-200 last:border-0"}>
            <button
              type="button"
              onClick={() => setOpenIdx(isOpen ? null : i)}
              className={`w-full flex items-center justify-between gap-4 py-4 text-left ${isOpen ? "text-[#0e7490]" : "text-slate-900"}`}
            >
              <span className={`font-semibold ${isOpen ? "text-[#0e7490]" : ""}`}>{it.question}</span>
