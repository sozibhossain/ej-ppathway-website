    ...
    <section className="py-16 md:py-20 bg-gradient-to-br from-[#0e7490] to-[#06495d] text-white">
      <div className="container-page grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          {title && <h2 className="text-3xl md:text-4xl font-bold leading-tight">{title}</h2>}
          {subtitle && <p className="mt-4 text-white/80 leading-relaxed max-w-xl">{subtitle}</p>}
        ...
        <div className="relative flex justify-center">
          {shots.length === 0 ? (
            <div className="w-64 h-[500px] rounded-3xl bg-white/10 border border-white/20" />
          ) : (
            <div className="flex gap-4 items-end">
              {shots.slice(0, 3).map((src, i) => (
                <div key={i} className={`relative w-44 ${i === 1 ? "h-[480px]" : "h-[400px]"} rounded-3xl overflow-hidden shadow-2xl ring-4 ring-white/20`}>
