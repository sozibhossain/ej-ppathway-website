    ...
    <>
      ...
      <section className="py-12 sm:py-16 md:py-20 bg-[#f0f9fb]">
        <div className="container-page text-center max-w-3xl">
          ...
          {hero.title && <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold text-[#0e7490]">{hero.title}</h1>}
          {hero.subtitle && <p className="mt-3 text-sm sm:text-base text-slate-600">{hero.subtitle}</p>}
        </div>
      </section>
      ...
      {data.commitment && (
        <section className="py-10 sm:py-14">
          <div className="container-page text-center">
            {data.commitment.title && <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#0e7490] mb-8 sm:mb-10">{data.commitment.title}</h2>}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
              {(data.commitment.cards || []).map((c, i) => (
                <div key={i} className="bg-[#f0f9fb] border border-[#cfe9f0] rounded-2xl p-4 sm:p-6 text-left">
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-[#0e7490] text-white inline-flex items-center justify-center shrink-0">
      ...
      {data.trustedByThousands && (
        <section className="py-10 sm:py-14 bg-gradient-to-br from-[#0e7490] to-[#06495d] text-white">
          <div className="container-page text-center">
            {data.trustedByThousands.title && <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-5 sm:mb-6">{data.trustedByThousands.title}</h2>}
      ...
      {data.fairResolution && (
        <section className="py-10 sm:py-14">
          <div className="container-page max-w-3xl text-center">
            {data.fairResolution.title && <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900">{data.fairResolution.title}</h2>}
            {data.fairResolution.subtitle && <p className="mt-3 text-sm sm:text-base text-slate-600">{data.fairResolution.subtitle}</p>}
      ...
      {data.contactBlock && (
        <section className="py-10 sm:py-14 bg-gradient-to-br from-[#0e7490] to-[#06495d] text-white">
          <div className="container-page text-center max-w-3xl">
            ...
            {data.contactBlock.title && <h2 className="text-xl sm:text-2xl md:text-3xl font-bold">{data.contactBlock.title}</h2>}
            {data.contactBlock.subtitle && <p className="mt-3 text-sm sm:text-base text-white/85">{data.contactBlock.subtitle}</p>}
            <div className="mt-5">
              <CmsCtaButton link={data.contactBlock.ctaPrimary} variant="white" size="md" />
            </div>
            <div className="mt-3 text-xs text-white/80 break-words">
