...
export default async function EthicalStandardsPage() {
  ...
  return (
    <>
      <section className="py-12 sm:py-16 md:py-20 bg-[url('/grid.svg')] bg-center bg-cover relative">
        <div className="container-page text-center max-w-3xl relative z-10">
          ...
          {hero.subtitle && <p className="mt-3 text-sm sm:text-base text-slate-700">{hero.subtitle}</p>}
          {hero.banner && (
      ...
      {data.commitment && (
        <section className="py-10 sm:py-14 bg-gradient-to-br from-[#0e7490] to-[#06495d] text-white">
          <div className="container-page text-center max-w-3xl">
            ...
            {data.commitment.title && <h2 className="text-xl sm:text-2xl md:text-3xl font-bold">{data.commitment.title}</h2>}
            {data.commitment.body && (
              <p className="mt-3 text-sm sm:text-base text-white/85 whitespace-pre-line leading-relaxed">{data.commitment.body}</p>
            )}
            <div className="mt-6 flex flex-col sm:flex-row sm:flex-wrap gap-3 justify-center">
              <CmsCtaButton link={data.commitment.ctaPrimary} variant="white" size="md" className="w-full sm:w-auto" />
              <CmsCtaButton link={data.commitment.ctaSecondary} variant="outline" size="md" className="!border-white !text-white hover:!bg-white/10 w-full sm:w-auto" />
            </div>
          </div>
        </section>
      )}
