            ...
            <div className="relative">
              {data.reachStats.image ? (
                <div className="relative aspect-[4/3] rounded-2xl overflow-hidden">
                  <Image src={data.reachStats.image} alt={data.reachStats.title || ""} fill className="object-cover" sizes="600px" unoptimized />
                </div>
              ) : (
        ...
        <section className="py-16 md:py-20">
          <div className="container-page grid lg:grid-cols-2 gap-10 items-center">
            ...
            {data.requirements.image ? (
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden">
                <Image src={data.requirements.image} alt={data.requirements.title || ""} fill className="object-cover" sizes="600px" unoptimized />
              </div>
            ) : (
              ...
              {(data.advisorTestimonials.videos || []).map((v, i) => (
                <div key={i} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                  <div className="relative aspect-video bg-slate-900">
                    {v.videoUrl ? (
                      <video src={v.videoUrl} poster={v.thumbnail} controls className="w-full h-full object-cover" />
                    ) : v.thumbnail ? (
                      <Image src={v.thumbnail} alt={v.name || ""} fill className="object-cover" sizes="400px" unoptimized />
                    ) : null}
                    {!v.videoUrl && (
                  ...
                  {v.name && <div className="px-4 py-3 text-left font-semibold text-slate-900">{v.name}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {data.beforeYouApply && (
        <section className="py-14 bg-gradient-to-br from-[#082e3a] to-[#0e5d75] text-white">
          <div className="container-page text-center">
            ...
            {data.beforeYouApply.footnote && <p className="mt-3 text-xs text-white/70">{data.beforeYouApply.footnote}</p>}
          </div>
        </section>
      )}
